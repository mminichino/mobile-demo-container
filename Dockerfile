...
ENV TZ=Etc/UTC
RUN apt-get update
RUN apt-get install -q -y runit numactl tzdata lsof lshw bzip2 jq git vim netcat sysstat apt-utils ca-certificates gnupg lsb-release net-tools python3 python3-dev python3-pip python3-setuptools cmake build-essential curl sudo software-properties-common make pkg-config libssl-dev
...
RUN SGW_ARCH=$(dpkg --print-architecture) \
    && curl -s -o /var/tmp/couchbase-server-enterprise.deb "https://packages.couchbase.com/releases/7.6.1/couchbase-server-enterprise_7.6.1-linux_${SGW_ARCH}.deb"
...
RUN pip3 install pyhostprep cbcmgr
...
# Add utilities that will be used to setup the demo
